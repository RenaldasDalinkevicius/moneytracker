...
class _EditExpensesPage extends State<EditExpensesPage> {
  Future openDialog({required ExpensesModel model, required String title, TextInputFormatter? format, TextInputType? type, bool? readOnly, bool? forDate}) {
    ...
    Future<void> update({required ExpensesModel model, required String snackbarText}) async {
      await Services().updateExpenses(widget.index, model);
        ...
        actions: [
          customFlatButton(text: "Update", onClicked: () async {
            if (editController.text.isNotEmpty) {
              if (title == "what") {
                ExpensesModel newModel = ExpensesModel(what: editController.text, price: model.price, date: model.date, note: model.note);
                await update(model: newModel, snackbarText: "Updated");
              } else if (title == "price") {
                ExpensesModel newModel = ExpensesModel(what: model.what, price: int.parse(editController.text), date: model.date, note: model.note);
                await update(model: newModel, snackbarText: "Updated");
              } else if (title == "date") {
                ExpensesModel newModel = ExpensesModel(what: model.what, price: model.price, date: DateTime.parse(editController.text), note: model.note);
                await update(model: newModel, snackbarText: "Updated");
              } else if (title == "note") {
                ExpensesModel newModel = ExpensesModel(what: model.what, price: model.price, date: model.date, note: editController.text);
                await update(model: newModel, snackbarText: "Updated");
              }
            }
  ...
  Widget build(BuildContext context) {
    return FutureBuilder(
      future: Services().getExpensesIndex(widget.index),
      builder: ((context, snapshot) {
        if (snapshot.connectionState == ConnectionState.done) {
          final ExpensesModel expense = snapshot.data!;
          return Scaffold(
            appBar: AppBar(
                            ...
                            children: [
                              Expanded(
                                child: customFlatButton(
                                  text: "Yes",
                                  onClicked: () async {
                                    Services().deleteExpenses(widget.index);
            ...
            body: ListView(
              children: [
                customListTile(
                  title: expense.what,
                  subtitle: "What is the expense",
                  onClicked: () => openDialog(model: expense, title: "what"),
                  trailing: Icon(Icons.edit)
                ),
                customListTile(
                  title: "${expense.price.toString()} ${getCurrency(context)}",
                  subtitle: "Price",
                  onClicked: () => openDialog(model: expense, title: "price", format: FilteringTextInputFormatter.digitsOnly, type: TextInputType.number),
                  trailing: Icon(Icons.edit)
                ),
                customListTile(
                  title: DateFormat('yyyy-MM-dd').format(expense.date),
                  subtitle: "Date",
                  onClicked: () => openDialog(model: expense, title: "date", forDate: true, readOnly: true),
                  trailing: Icon(Icons.edit)
                ),
                customListTile(
                  title: expense.note??"",
